  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      const videoConstraints: MediaTrackConstraints = {
        // 새 웹캠 기준 최대 지원 해상도 1280x720 (1920x1080은 캡 또는 브라우저 제약으로 미지원)
        width: { min: 640, ideal: VIDEO_WIDTH, max: VIDEO_WIDTH },
        height: { min: 360, ideal: VIDEO_HEIGHT, max: VIDEO_HEIGHT },

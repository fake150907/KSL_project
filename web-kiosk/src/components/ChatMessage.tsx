    ...
    <div className={`flex items-end gap-2 ${isAgent ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`flex min-w-0 max-w-[75%] flex-col gap-1 ${isAgent ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`
          ...
          ${isAgent
            ? dark ? 'bg-[#2563eb] text-white rounded-tr-sm border border-[#60a5fa]/30' : 'bg-[#2563eb] text-white rounded-tr-sm border border-[#93c5fd]'
            : dark ? 'bg-[#14b8a6] text-[#042f2e] rounded-tl-sm' : 'bg-[#ccfbf1] text-[#134e4a] rounded-tl-sm border border-[#99f6e4]'}
        `}>
          {message.text}
        </div>

...
def predict_dual_scenario(
    ...
    pair_candidates = [
        item for item in lookup_candidates
        if item.get("source") == "word_sentence_pair"
    ]
    # In the resident demo a weak-but-present WORD+SEN pair is often safer than
    # an unrelated single sentence with a higher raw confidence.
    if pair_candidates:
        ...
        if (
            pair_word_conf >= 0.12 and
            pair_sentence_conf >= 0.12 and
            best_pair_score >= best_single_score * 0.55
        ):
            lookup_candidates = [
                best_pair,
